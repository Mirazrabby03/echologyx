...
sessionStorage.setItem('burgerMenuTriggered', 'false');
function mainCss() {
...
.mobile-nav__item.custom-gutschein {
	opacity: 1 !important;
}
...
function TrackCart(body) {
	if (window.innerWidth <= 1198) {
		...
		const callback = function(mutationsList, observer) {
			if (document.querySelector('mobile-navigation') && !document.querySelector('mobile-navigation').classList.contains('apf-04-test')) {
				document.querySelector('mobile-navigation').classList.add('apf-04-test');
				waitForElem('button[aria-controls="mobile-menu-4"]', mainJs);
			}
		};
