...
function detectCartItemChange() {
	...
	const callback = function() {
		if (Number(document.querySelectorAll('.Cart__ItemList .CartItem__Wrapper').length) !== cartItems) {
			...
			if(isMobile){
				console.log('executing recomendationItemArrangementMobile');
				// setTimeout(() => {
...
// function removeItemFromSLider(productID) {
// 	if(!swiperOBJ || !productID) return;
//     console.log('passed to remove', productID);

//     // Find the slide element with the matching product ID
//     // const slideElement = document.querySelector(`.ab--recomendation-panel__product[data-id="${productID}"]`);
// 	// console.log('need to remove',slideElement)
//     // if (slideElement) {
//     //     const dataIndex = slideElement.getAttribute('data-index');
// 	// 	console.log('dataIndex',dataIndex)	
//     //     // const slideIndex = parseInt(dataIndex);
// 	// 	const slideIndex = Number(dataIndex);
// 	// 	// console.log('slideIndex',slideIndex)
		
// 	// 		console.log(typeof slideIndex,'slideIndex')
		
// 	// 	swiperOBJ.removeSlide(slideIndex);
        
//     // }

// 	document.querySelectorAll('.ab--recomendation-panel__product').forEach((elm, i) => {
// 		if(elm === document.querySelector(`.ab--recomendation-panel__product[data-id="${productID}"]`)){
// 			console.log('need to remove------------',elm)
// 			// elm.remove();
...
function recomendationItemArrangementMobile() {
	...
	console.log('removedItemFromCart',removedItemFromCart)
	...
	products.forEach(product => {
		...
		if(!document.querySelector(`.ab--recomendation-panel__product[data-id="${product.id}"]`)){
			...
			console.log(newCartItem, product.id)
			console.log(newCartItem.includes(product.id), 'iffffffffffffffff')
	...
	// setTimeout(() => {
		...
		for (let index = recomendationItems.length - 1; index >= 0; index--) {
			...
			if (availableRecommendationItems > 3) {	
				console.log(availableRecommendationItems,'availableRecommendationItems')		
				if(item.getAttribute('data-id') !== removedItemFromCart[0]){
					console.log(item.getAttribute('data-id') !== removedItemFromCart[0])
					console.log(item.getAttribute('data-id'), removedItemFromCart[0])
					console.log(item.getAttribute('data-id'),'removeddddd initially-------')
...
// const totalSec = 3;
// let currentSec = 0;
// (function waitForSwiperForFiveSec(){
// 	if(window.Swiper){
// 		mainJs();
// 	}else if(currentSec < totalSec){
// 		setTimeout(() => {
// 			currentSec = currentSec + 0.1;
// 			waitForSwiperForFiveSec();
// 		}, 100);
// 	}else{
// 		mainJs();
// 	}
