...
function recomendationItemArrangementMobile() {
    ...
    setTimeout(() => {
	document.querySelectorAll('.ab--recomendation-panel__product').forEach(item => {
		if(item.getAttribute('aria-label') === '4 / 4'){
			removeItemFromSLider(item.getAttribute('data-id'));
		}
	})
	}, 1000);
...
function recomendationItemArrangement() {
    ...
    // recomendationItems.forEach((item) => {
